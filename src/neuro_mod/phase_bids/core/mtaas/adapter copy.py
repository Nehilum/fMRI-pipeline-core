...
from neuro_mod.phase_bids.core.models import ConversionEntry, ProcessStatus
...
# 定义一个简单的 dataclass 来存结果，只在这个文件用，或者只是 dict 也行
@dataclass
class MtaasResult:
...
class MtaasAdapter:
    def __init__(self, config: Dict):
        ...
        # --- 用于保存计算结果以便生成 CSV 报告 ---
        self.buckets: Dict[str, AuditBucket] = {}
        self.match_results = {} 
        self.verdicts = {}      
        
        # [新增] 用于存储从 CSV 读取的 {filename: task_name} 映射
        self.task_mapping: Dict[str, str] = {}

    def run_audit(self, entries: List[ConversionEntry]) -> None:
        """
        [修改] 不再修改 entries，而是返回一个 {scan_id: result} 的字典
        """
        logger.info(f"Starting MTAAS Audit (Read-Only Mode)...")
        ...
        for key, bucket in self.buckets.items():
            ...
            # === [关键修改] 生成结果字典，而不是修改 Entry ===
            for i, mri_evt in enumerate(bucket.mri_events):
                scan_id = mri_evt.scan_id
                
                # 查找是否匹配
                matched_pair = next((p for p in match_result.pairs if p[0] == i), None)
                ...
                if matched_pair:
                    ...
                    # offset = mri_end_time - log_time
                    mri_time = m_times[matched_pair[0]] 
                    resp_time = r_times[resp_idx]
                    res.offset = mri_time - resp_time
                    ...
                    if verdict.status == "PASS":
                        res.comments = f"Matched: {resp_evt.filename}"
                    else:
                        res.comments = f"Matched (Bucket {verdict.status})"
                else:
    ...
    def _load_task_mapping(self, csv_path: str):
        """
        [新增] 读取 CSV 表格，构建 文件名 -> Task列 的映射字典。
        """
        self.task_mapping = {}
        try:
            # utf-8-sig 处理可能存在的 BOM 头
            with open(csv_path, 'r', encoding='utf-8-sig') as f:
                reader = csv.DictReader(f)
                for row in reader:
                    # 尝试兼容常见的“文件名”列名，如果 CSV 列名确定是 'FileName' 可直接写死
                    # 这里的逻辑是尝试获取文件名，然后获取 'Task' 列
    ...
    def generate_csv_report(self, output_path: str):
        """
        生成 audit_truth_table_scans.csv
        """
            ...
            for m_idx, m_evt in enumerate(bucket.mri_events):
                ...
                if matched_pair:
                    ...
                    if len(matched_pair) >= 3:
                        offset = matched_pair[2]
                    else:
                        # 重新获取对应的时间对象进行计算
                        # 注意：generate_csv_report 里没有 m_times/r_times 局部变量
                        # 需要直接从 event 对象里取
    ...
    def _apply_verdict(self, bucket, match_result, verdict, entry_map):
        for i, mri_evt in enumerate(bucket.mri_events):
            entry = entry_map.get(mri_evt.scan_id)
            ...
            if matched_pair:
                ...
                entry.mtaas_status = "MATCHED"
                entry.mtaas_offset = matched_pair[2]
                
                # [修改] 从 task_mapping 字典中获取 Task Name
                # 如果字典里找不到，回退到 "unknown" 或文件名中的部分信息
                task_name = self.task_mapping.get(resp_evt.filename, "unknown")
                
                entry.target.task = task_name 
                
                if verdict.status == "PASS":
                     entry.quality_check.passed = True
                     entry.quality_check.comments = f"Matched: {resp_evt.filename} (Task: {task_name})"
                else:
                     entry.quality_check.comments = f"Matched (Bucket {verdict.status}): {resp_evt.filename}"
            else:
                entry.mtaas_status = "UNMATCHED"
